...
def figure(gid, edges, nodes, positions, translate=str):
    ...
    for row in edges.itertuples():
        ...
        fig.add_trace(go.Scatter(x=[(x0+x1)/2], y=[(y0+y1)/2], mode='markers', marker=dict(size=10, color='rgba(138,155,168,0.35)'), text=[f'{row.src} → {row.dst}<br>{row.sum_kzt:,.2f} KZT · {row.n_tx} {translate("Переводов")}'], hovertemplate='%{text}<extra></extra>', showlegend=False))
    ...
    fig.add_trace(go.Scatter(x=[positions[g][0] for g in gids], y=[positions[g][1] for g in gids], mode='markers', text=[f'gid {g}<br>{translate(labels[indexed.loc[g, "role"]])}<br>{translate("Приоритет проверки")} {indexed.loc[g, "priority_score"]:.3f}' for g in gids], hovertemplate='%{text}<extra></extra>', marker=dict(size=[22 if g == gid else 13 for g in gids], color=[colors[indexed.loc[g,'role']] for g in gids], line=dict(width=2,color='white')), showlegend=False))
